...
fn default_poll_interval() -> u32 { 500 }
fn default_log_level() -> String { "info".into() }
fn default_socket_path() -> String { "/var/run/fand.sock".into() }
fn default_socket_mode() -> u16 { 0o600 }
fn default_lp_attenuation() -> f32 { 1.0 }
fn default_fusion() -> String { "max".into() }
fn default_hysteresis_up() -> f32 { 1.0 }
fn default_hysteresis_down() -> f32 { 3.0 }
fn default_smoothing_alpha() -> f32 { 0.25 }
fn default_ramp_down() -> u32 { 600 }
fn default_panic_temp() -> f32 { 95.0 }
fn default_panic_hold() -> u32 { 10 }

#[derive(Debug)]
pub enum ValidationError {
    TomlSyntax { line: usize, col: usize, message: String },
    MissingRequired { field: String, fan_index: Option<u8> },
    DuplicateFanIndex { index: u8 },
    UnknownFanIndex { index: u8, available: Vec<u8> },
    EmptySensors { fan_index: u8 },
    UnknownSensor { sensor: String, available: Vec<String> },
    CurveTooShort { fan_index: u8, count: usize },
    CurveTooLong { fan_index: u8, count: usize },
    NonMonotoneTemp { fan_index: u8, bp: usize, prev_temp: f32, temp: f32 },
    NonMonotoneRpm { fan_index: u8, bp: usize, prev_rpm: u32, rpm: u32 },
    RpmOutOfRange { fan_index: u8, bp: usize, rpm: u32, max: f32 },
    HysteresisInverted { fan_index: u8, up: f32, down: f32 },
    SmoothingAlphaRange { fan_index: u8, alpha: f32 },
    PanicTempTooLow { fan_index: u8, panic: f32, last_curve_temp: f32 },
    LowPowerRange { value: f32 },
    InvalidSocketMode { mode: u16 },
    FileTooLarge { size_bytes: u64 },
    NestingTooDeep { depth: usize },
    UnknownField { key: String, context: String },
    UnsafePermissions { path: String, owner: u32, mode: u32 },
}
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::CurveTooShort { fan_index, count } => {
                write!(f, "fan[{fan_index}]: curve has {count} breakpoints (minimum 2)")
            }
            Self::CurveTooLong { fan_index, count } => {
                write!(f, "fan[{fan_index}]: curve has {count} breakpoints (maximum 100)")
            }
            Self::NonMonotoneTemp { fan_index, bp, prev_temp, temp } => {
                write!(f, "fan[{fan_index}]: breakpoint {bp} temp ({temp}°C) ≤ previous ({prev_temp}°C)")
            }
            Self::NonMonotoneRpm { fan_index, bp, prev_rpm, rpm } => {
                write!(f, "fan[{fan_index}]: breakpoint {bp} RPM ({rpm}) < previous ({prev_rpm})")
            }
            Self::RpmOutOfRange { fan_index, bp, rpm, max } => {
                write!(f, "fan[{fan_index}]: breakpoint {bp} RPM ({rpm}) exceeds max ({max})")
            }
            Self::HysteresisInverted { fan_index, up, down } => {
                write!(f, "fan[{fan_index}]: hysteresis_down ({down}) must be ≥ hysteresis_up ({up})")
            }
            Self::SmoothingAlphaRange { fan_index, alpha } => {
                write!(f, "fan[{fan_index}]: smoothing_alpha ({alpha}) must be in (0.0, 1.0]")
            }
            Self::PanicTempTooLow { fan_index, panic, last_curve_temp } => {
            ...
            Self::UnsafePermissions { path, owner, mode } => {
                write!(f, "{path}: unsafe permissions (owner uid {owner}, mode {mode:#o})")
            }
        }
    }
}
