...
- Conventional Commits + DCO (`git commit -s`) + `Co-Authored-By` trailer
- Worktree-first per `~/.claude/CLAUDE.md` — branches named `NNN-slug`, worktree dirs end in `-NNN-slug`
...
- `sonar.yml` — SonarQube on `sonarqube.home301server.com.br` (project token scoped)

## Cross-references

- **Vault canon (DO NOT duplicate):** `~/.claude/CLAUDE.md`, `~/Documents/Code/CLAUDE.md`, `~/Documents/Notes/2. Areas/👷 Work/CLAUDE.md`
- **Constitution:** `~/Documents/Code/yolo-labz/.specify/memory/constitution.md` v1.0.0
- **Audit doc:** `phsb5321/notes-work#24` → `YOLO-LABZ-AUDIT-2026-05-27.md`
- **Active spec:** `024-yolo-labz-portfolio-consolidation-2026Q2`
...
## Active feature work pointers (cross-reference only — DO NOT duplicate scope)

Open PRs at time of writing:

- [#16](https://github.com/yolo-labz/fand/pull/16) — SHA-pin all actions + `attest-build-provenance` v4.1.0 + `permissions: {}` deny-all
- [#17](https://github.com/yolo-labz/fand/pull/17) — lefthook + git-cliff + cargo-deny in CI
- [#18](https://github.com/yolo-labz/fand/pull/18) — dual SBOM (CycloneDX 1.7 + SPDX 2.3) + Scorecard + harden-runner audit
